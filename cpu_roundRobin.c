...
void readInputs(int n, int *bsum, int arr[][4])
{
    for (int i = 0; i < n; i++)
    {
...
void calculatePerformance(int n, int bsum, int arr[][4], struct performance_parameters pp[])
{
...
void main()
{
    ...
    // arr[x][3] initially set to 0, used tocount the number of times CPU allocated for the process
    readInputs(n, &bsum, arr);
    struct performance_parameters pp[n];
    calculatePerformance(n, bsum, arr, pp);
    printResult(n, pp);
}
